...
- **app_schemes.json** - 包含跨平台常见应用的 URL Schemes（微信、QQ、支付宝、微博、知乎、美团、滴滴打车等）
- **LICENSE** - 项目许可证文件
...
// 获取常用应用的 URL Scheme
const appSchemes = JSON.parse(fs.readFileSync('app_schemes.json', 'utf8'));
const wechatScheme = appSchemes.common_apps.find(app => app.app_name === '微信');
...
在 iOS 快捷指令中，可以使用 `ios_schemes.json` 和 `app_schemes.json` 中的 URL Scheme 创建深度链接操作，实现应用间的无缝跳转：
